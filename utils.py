...
def cal_download_time():
	global start_time
	global download_time
	global download_stop
	while not download_stop:
		time.sleep(1)
		download_time = time.time() - start_time
		gui.GUIOperate.update_download_time(download_time)
...
def download_m3u8(m3u8_url, path, name, video_count=''):
	...
	try:
		...
		th_download_time = threading.Thread(target=cal_download_time)
