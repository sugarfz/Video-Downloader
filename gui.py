...
class GUIOperate(object):
	...
	def update_progress_bar(sum_time, now_time):
		...
		if abs(percent - 100) < 0.5:
			percent = 100
			green_length = sum_length + 1
		var_progress_bar_percent.set('%.2f  %%' % percent)
		canvas_progress_bar.coords(canvas_shape, (0, 0, green_length, 25))

	# 更新下载时间
	@staticmethod
	def update_download_time(download_time):
		hour = int(download_time/3600)
		minute = int(download_time/60) - hour*60
		second = download_time % 60
...
# 顶层窗口
top = Tk()  # 创建顶层窗口
top.title('视频下载器 - V1.0    by:  sugarfz')
...
canvas_shape = canvas_progress_bar.create_rectangle(0, 0, 0, 25, fill='#06B025', outline='#06B025')
canvas_text = canvas_progress_bar.create_text(286, 4, anchor=NW)
...
# 进度条
canvas_progress_bar.place(relx=0.461, rely=0.952, anchor=CENTER)
...
scrolled_text.insert(END, '等待下载任务...\n')
scrolled_text.see(END)

var_progress_bar_percent.set('00.00  %')
# canvas_progress_bar.coords(canvas_shape, (0, 0, sum_length/2, 25))
